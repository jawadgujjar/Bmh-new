import SubAboutdigital from "@/components/digital-marketing/sub-category-digital/subaboutdigital"; // Replace with web-specific if needed
...
import SubHeroDigitalMarketing from "@/components/digital-marketing/sub-category-digital/subdigitalhero"; // Replace with web-specific if needed
...
async function getSubCategoryData(slug) {
  try {
    console.log(`Fetching data for slug: ${slug}, category: web-development`);
    const res = await fetch(
      `${process.env.NEXT_PUBLIC_API_URL || "http://localhost:3000"}/api/subcategories?slug=${slug}&category=web-development`,
      { cache: "no-store" }
    );
...
export default async function SubCategoryPage({ params }) {
  const { slug } = params;
  console.log(`Rendering page for slug: ${slug} under webdevelopment`);
