...
const SectionItem = ({
  ...
  const headingValue = Form.useWatch(["sections", field.name, "heading"], form);
  ...
  return (
    ...
    >
      ...
      <Form.Item
        ...
        rules={[
          { required: true, message: "Section heading is required" },
          { min: 3, message: "Section heading must be at least 3 characters" }
        ]}
      >
      ...
      {layout !== "description-and-form" && (
        <Form.Item
          ...
          label="Description"
          rules={[
            { required: true, message: "Section description is required" },
            { min: 10, message: "Section description must be at least 10 characters" }
          ]}
        >
          <TiptapEditor
      ...
      {(layout === "image-left" || layout === "image-right") && (
        <Form.Item
          ...
          rules={[{ required: true, message: "Section image is required for this layout" }]}
        >
          <ImageUploadField 
            label="Section Image" 
            required={true}
...
export default function Pages() {
  ...
  const handleOk = async () => {
    try {
      ...
      const validationErrorList = validateFormFields(values);
      ...
      if (validationErrorList.length > 0) {
        ...
        message.error({
          content: `Please fix ${validationErrorList.length} error(s) before saving`,
          duration: 3,
        });
        return;
      }
              ...
              items={[
                {
                  ...
                  children: (
                    <>
                      <Row gutter={16}>
                        <Col span={12}>
                          <Form.Item
                            ...
                            name="title"
                            rules={[
                              { required: true, message: "⚠️ Page title is required" },
                              { min: 3, message: "⚠️ Page title must be at least 3 characters" }
                            ]}
                          >
                            <Input 
                        ...
                        <Col span={12}>
                          <Form.Item
                            ...
                            name="slug"
                            rules={[
                              { required: true, message: "⚠️ URL slug is required" },
                              { pattern: /^[a-z0-9-]+$/, message: "⚠️ Slug can only contain lowercase letters, numbers, and hyphens" }
                            ]}
                          >
                      ...
                      <Row gutter={16}>
                        <Col span={12}>
                          <Form.Item
                            label="Category"
                            name="category"
                            rules={[{ required: true, message: "⚠️ Please select a category" }]}
                          >
                        ...
                        <Col span={12}>
                          <Form.Item
                            label="SubCategory"
                            name="subcategory"
                            rules={[{ required: true, message: "⚠️ Please select a subcategory" }]}
                          >
                      ...
                      >
                        <Input.TextArea rows={3} placeholder="Enter short description (max 200 characters)" showCount maxLength={200} />
                      </Form.Item>
                    </>
                  ),
                },
                ...
                },
                {
                  ...
                  children: (
                    <>
                      <Alert
                        message="Content Sections Requirements"
                        description={
                          <ul style={{ margin: 0, paddingLeft: 20 }}>
                            <li>✓ At least one section is required</li>
                            <li>✓ Each section must have a heading (min 3 characters)</li>
                            <li>✓ Each section must have description (min 10 characters) or description points</li>
                            <li>✓ For "Image Left/Right" layouts, an image is required</li>
                            <li>✓ For "Description & Form" layout, at least one description point is required</li>
                          </ul>
                        }
                        type="info"
                        showIcon
                ...
                },
                {
                  ...
                  children: (
                    <>
                      <Alert
                        message="FAQs Requirements"
                        description="Each FAQ must have both a question and an answer"
                        type="info"
                        showIcon
                        style={{ marginBottom: 16 }}
                      />
                  ...
                  children: (
                    <Card size="small" style={{ background: "#f5f5f5" }}>
                      ...
                      <Form.Item name={["seo", "metaDescription"]} label="Meta Description">
                        <Input.TextArea rows={3} maxLength={160} showCount placeholder="Enter meta description (recommended: 150-160 characters)" />
                      </Form.Item>

                      <Form.Item name={["seo", "metaKeywords"]} label="Meta Keywords">
                        <Input placeholder="keyword1, keyword2, keyword3 (comma separated)" />
                        <Text type="secondary" style={{ fontSize: 12 }}>Separate keywords with commas</Text>
                      </Form.Item>

                      <Form.Item name={["seo", "schemaMarkup"]} label="Schema Markup">
                        <Input.TextArea rows={5} placeholder='{"@context": "https://schema.org", "@type": "WebPage"}' />
                        <Text type="secondary" style={{ fontSize: 12 }}>Enter valid JSON-LD schema markup</Text>
                      </Form.Item>
                    </Card>
                  ),
                },
